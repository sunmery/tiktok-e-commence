...
func RegisterOrderServiceHTTPServer(s *http.Server, srv OrderServiceHTTPServer) {
	r := s.Route("/")
	r.POST("/v1/order/place", _OrderService_PlaceOrder0_HTTP_Handler(srv))
	r.GET("/v1/order/list", _OrderService_ListOrder0_HTTP_Handler(srv))
	r.POST("/v1/order/mark_paid", _OrderService_MarkOrderPaid0_HTTP_Handler(srv))
}
